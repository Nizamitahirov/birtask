...
export function ProjectForm({ initial, onSubmit, onCancel, loading }: ProjectFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-text-secondary text-xs mb-1.5">Rəhbər</label>
        <input
          value={form.owner}
        ...
        <div className="flex gap-2">
          {PROJECT_COLORS.map(c => (
            <button
              ...
              onClick={() => set('color', c)}
              className={cn(
                'w-7 h-7 rounded-lg transition-all duration-200 flex-shrink-0',
                form.color === c && 'ring-2 ring-white/50 scale-110'
              )}
              style={{ background: c }}
            />
          ))}
