...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ success: true, data: omitPassword({ ...doc.data(), id: doc.id }) })
  } catch (err: unknown) {
...
export async function PUT(
  ...
  try {
    ...
    await adminDb.collection('activity').add({
      action: 'update', entityType: 'user', entityId: params.id,
      entityName: data.displayName || updated?.displayName || '',
      userId: 'system', userDisplayName: 'System', createdAt: now,
    })

    return NextResponse.json({ success: true, data: omitPassword({ ...updated, id: params.id }) })
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : 'Bilinməyən xəta'
    return NextResponse.json({ success: false, error: message }, { status: 500 })
  }
...
export async function DELETE(
  ...
  try {
    ...
    await adminDb.collection('activity').add({
      action: 'delete', entityType: 'user', entityId: params.id,
      entityName: displayName, userId: 'system', userDisplayName: 'System', createdAt: now,
    })
